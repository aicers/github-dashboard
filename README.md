...
1. Build for the target platform and tag the image:
   ...
   ```bash
   docker buildx build --platform linux/amd64 -t github-dashboard:0.3.1 --load .
   ```

   > Replace `github-dashboard:0.3.1` with the tag you plan to deploy. The
...
3. Export the image to a tarball and transfer it to the server (copy your
   ...
   ```bash
   docker save github-dashboard:0.3.1 -o github-dashboard-0.3.1.tar
   scp github-dashboard-0.3.1.tar user@server:/path/to/github-dashboard/
   ```
...
4. On the server, load the tarball, install the HTTPS certificate, and restart
   ...
   ```bash
   docker compose down
   docker load -i /path/to/github-dashboard/github-dashboard-0.3.1.tar
   ...
   > by `docker run ... github-dashboard:0.3.1` instead of the Compose commands.
...
1. Load the image and create a shared network:
   ...
   ```bash
   docker load -i github-dashboard-0.3.1.tar
