...
import type { BackfillResult, DashboardStats, SyncStatus } from "@/lib/sync/service";
...
export function DashboardClient({ status, stats }: DashboardClientProps) {
  ...
  const [autoEnabled, setAutoEnabled] = useState(config?.auto_sync_enabled ?? false);
  ...
  async function handleOrgUpdate() {
    startUpdateOrg(async () => {
      try {
        ...
        setFeedback(
          error instanceof Error ? error.message : "구성 업데이트 중 오류가 발생했습니다.",
        );
      }
    });
  }
    ...
    startBackfill(async () => {
      try {
        ...
        if (report) {
          ...
          const failedChunk = report.chunks.find((chunk) => chunk.status === "failed");
        ...
        setFeedback(error instanceof Error ? error.message : "백필 중 오류가 발생했습니다.");
      }
    });
  }
  ...
  async function handleAutoToggle(nextEnabled: boolean) {
    startToggleAuto(async () => {
      try {
        ...
        setFeedback(nextEnabled ? "자동 동기화를 실행했습니다." : "자동 동기화를 중단했습니다.");
        router.refresh();
      } catch (error) {
        setFeedback(
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-8 px-6 py-12">
      <header className="flex flex-col gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          조직({orgName || config?.org_name})의 활동 데이터를 수집하고 동기화 상태를
          관리합니다.
        </p>
      ...
      <section className="grid gap-6 md:grid-cols-2">
        <Card className="border-border/70">
          ...
          <CardContent className="flex flex-col gap-4">
            ...
            <label className="flex flex-col gap-2 text-sm" htmlFor={intervalInputId}>
        ...
        <Card className="border-border/70">
          <CardHeader>
            ...
            <CardDescription>선택한 날짜부터 최신 데이터까지 즉시 수집합니다.</CardDescription>
          </CardHeader>
          <CardContent className="flex flex-col gap-4">
            <label className="flex flex-col gap-2 text-sm" htmlFor={backfillInputId}>
        ...
        {backfillHistory.length > 0 && (
          <Card className="border-border/70">
            <CardHeader>
              ...
              <CardDescription>최근 실행 최대 5회까지의 누적 결과를 확인할 수 있습니다.</CardDescription>
            </CardHeader>
                ...
                >
                  <div className="flex flex-wrap gap-4 text-muted-foreground">
                    <span className="font-medium text-foreground">
                      실행 #{backfillHistory.length - index} • {formatRange(report.startDate, report.endDate)}
                    </span>
                      ...
                      chunk.status === "success" ? (
                        ...
                        >
                          ...
                          <span className="text-muted-foreground">
                            이슈 {chunk.summary.counts.issues.toLocaleString()} / PR {" "}
                            {chunk.summary.counts.pullRequests.toLocaleString()} / 리뷰 {" "}
                            {chunk.summary.counts.reviews.toLocaleString()} / 댓글 {" "}
        ...
        <Card className="border-border/70">
          ...
          <CardContent className="space-y-3 text-sm text-muted-foreground">
            ...
            <p>마지막 성공: {formatDateTime(config?.last_successful_sync_at)}</p>
          </CardContent>
          <CardFooter className="gap-3">
            ...
            >
              {isTogglingAuto ? "처리 중..." : autoEnabled ? "자동 동기화 중단" : "자동 동기화 시작"}
            </Button>
          </CardFooter>
        </Card>
        ...
        <Card className="border-destructive/30">
          <CardHeader>
            ...
            <CardDescription>
              문제 발생 시 저장된 GitHub 데이터를 모두 삭제합니다. 로그는 유지됩니다.
            </CardDescription>
          </CardHeader>
      ...
      <section className="flex flex-col gap-6">
        <Card className="border-border/70">
          <CardHeader>
            ...
            <CardDescription>데이터 규모와 시간 범위를 한눈에 확인합니다.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-6">
            <div className="grid gap-4 sm:grid-cols-4">
              {["issues", "pull_requests", "reviews", "comments"].map((key) => (
                <div key={key} className="rounded-lg border border-border/60 bg-background px-4 py-3">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-lg border border-border/60 bg-background px-4 py-3 text-sm">
                <p className="text-xs uppercase text-muted-foreground">이슈 수집 범위</p>
                <p className="mt-2">{formatDateTime(stats.issuesRange?.oldest)}</p>
                <p className="text-xs text-muted-foreground">
                  최근 업데이트: {formatDateTime(stats.issuesRange?.newest)}
                </p>
              </div>
              <div className="rounded-lg border border-border/60 bg-background px-4 py-3 text-sm">
                <p className="text-xs uppercase text-muted-foreground">PR 수집 범위</p>
                <p className="mt-2">{formatDateTime(stats.pullRequestsRange?.oldest)}</p>
                <p className="text-xs text-muted-foreground">
                  최근 업데이트: {formatDateTime(stats.pullRequestsRange?.newest)}
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div>
                ...
                <ul className="mt-2 space-y-2 text-sm text-muted-foreground">
                  ...
                  {stats.topUsers.map((row) => {
                    ...
                    const label = profile?.login ?? row.authorId ?? "(알 수 없음)";

                    return (
                      <li key={row.authorId ?? label} className="flex justify-between gap-4">
                        <span>
                          {label}
                          {profile?.name && (
              ...
              <div>
                ...
                <ul className="mt-2 space-y-2 text-sm text-muted-foreground">
                  {stats.topRepositories.length === 0 && <li>데이터가 없습니다.</li>}
                  {stats.topRepositories.map((row) => {
                    const issueCount = row.issueCount;
                    const prCount = row.pullRequestCount;
                    const repo = row.repository;
                    const label = repo?.nameWithOwner ?? repo?.name ?? row.repositoryId ?? "(알 수 없음)";

                    return (
                      <li key={row.repositoryId ?? label} className="flex justify-between gap-4">
                        <span>{label}</span>
                        <span>
                          이슈 {issueCount.toLocaleString()} / PR {prCount.toLocaleString()}
                        </span>
                      </li>
                    );
                  })}
        ...
        <Card className="border-border/70">
          <CardHeader>
            ...
            <CardDescription>각 리소스의 실행 상태와 메시지를 확인합니다.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            {status.logs.length === 0 && <p className="text-muted-foreground">로그가 없습니다.</p>}
            {status.logs.map((log) => (
              <div
                key={log.id}
              ...
              >
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {formatDateTime(log.started_at)} → {formatDateTime(log.finished_at)}
                </p>
